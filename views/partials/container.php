        ...
        <div class="shadow sm:overflow-hidden sm:rounded-md">
          <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
            <?php foreach ($inputFields['fields'] as $fieldName) : ?>
              <?php if ($fieldName === 'bl_id') : ?>
                <input type="hidden" name="<?= $fieldName ?>" value="<?= $blId ?>" />
              <?php elseif ($fieldName === 'qte') : ?>
                <label for="<?= $fieldName ?>" class="block text-sm font-medium text-gray-700"><?= ucfirst($fieldName) ?></label>
                <div class="mt-1">
                  <input type="number" id="<?= $fieldName ?>" name="<?= $fieldName ?>" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                </div>
